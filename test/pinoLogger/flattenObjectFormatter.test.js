...
describe("Create instance of PinoLogger and test its flattenObject formatter.", function () {
  ...
  before(async function () {
    ...
    pinoStreamLogger = await pinoLoggerCreate(import.meta.url, { formatterType: 'flattenObject', stream: logStream });
    pinoFileLogger = await pinoLoggerCreate(import.meta.url, { formatterType: 'flattenObject', destination: path.join(logDir, 'info.log') });
  
    console.log('Loggers created.');
  });


  it("tests the flattenObject formatter.", async function () {
    this.timeout(MAX_TIMEOUT);
  ...
  it("tests the flattenObject formatter with a structured object.", async function () {
    this.timeout(MAX_TIMEOUT);
  ...
  it("tests flattenObject formatter logging to a file.", async function () {
    this.timeout(MAX_TIMEOUT);
